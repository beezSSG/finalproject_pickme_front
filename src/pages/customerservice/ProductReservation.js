...
function ProductReservation() {
    ...
    const handleCheckboxChange = (event, product, index) => {
        ...
        if (event.target.checked && !isNaN(quantity) && quantity > 0) {
            // 이미 선택된 상품인지 확인
            const existingProductIndex = selectedProducts.findIndex(item => item.name === product.name);
            ...
            <div className="mx-auto px-20 sm:px-3">
                ...
                <div>
                    <div className="text-center">
                        ...
                        <button className="bg-sub-yellow hover:bg-yellow-500 rounded-xl p-2 font-bold w-[100px] h-full" onClick={choiceProduct}>
                            상품 찾기
                        </button> 
                    </div><br/>
                ...
                {showproductModal && (
                    <div className="fixed inset-0 flex items-center justify-center bg-gray-400 bg-opacity-65">
                        <div className="w-[70%] h-screen pt-36 pb-20 sm:pb-5">
                            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full h-full bg-white overflow-y-auto" id="modalScrollableDiv">
                                {/* 모달 제목 */}
                                <div className="relative text-center my-5 text-3xl">
                ...
                <div className="text-right mt-3"><button className="bg-gray-700 rounded-xl p-3 text-white" onClick={consolee} >결제하기</button></div>
