...
function ContactUsDetail() {
  ...
  const ccbcommentlist = async () => {
    await axios
      .get("manager/ccbcommentlist", { params: { ccbId: id } })
      .then((resp) => {
  ...
  const ccbcommentwrite = async () => {
    ...
    await axios
      .get("manager/ccbcommentwrite", {
        params: { ccbId: id, managerId: managerId, content: coContent },
      })
      .then((resp) => {
        // alert(resp.data);
        setCommentList(resp.data);
  ...
  const ccbcommentdelete = async (ccbaid) => {
    await axios
      .get("manager/ccbcommentdelete", { params: { ccbId: id, id: ccbaid } })
      .then((resp) => {
        console.log(resp.data);
        setCommentList(resp.data);
      })
  ...
  async function ccbdetail() {
    await axios
      .get("manager/ccbdetail", { params: { id: id } })
      .then(function (resp) {
        console.log(resp.data);
          ...
          <table className="mt-8 w-full">
            <tbody>
              <tr className="border-t-2  border-black">
                <td
                  colSpan={4}
                  className="px-4 py-6 bg-gray-200 font-bold text-xl"
                >
                  {ccbdetailS.title}
                </td>
              </tr>
              ...
              <tr className="border-b-2 border-gray-400">
                <th className="py-3 bg-gray-200  text-center text-gray-700">
                  작성일
                </th>
                      ...
                      {adminName === "하기성" && (
                        <div className="py-3 pr-7">
                          <button
                            ...
                            onClick={() => ccbcommentdelete(list.id)}
                          >
                            삭제
                          </button>
                        </div>
