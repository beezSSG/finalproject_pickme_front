...
function ManagerPurchaseOrder() {
  ...
  function pohandleChange(id) {
    axios
      .get("manager/purchaseorderapprove", {
        params: { id: id, storeName: category },
      })
      .then(function (resp) {
        console.log(resp.data);
        setPurchaseorderList(resp.data.purchaseorderList);
    ...
    <>
      <div className="flex flex-row">
        ...
        <div className="max-w-full mx-[100px]">
          <div className="font-bold text-3xl flex items-center mt-[50px]">
            <IoReceiptOutline className="mr-4" />
            발주 목록
          </div>
          <br />
          <div className="text-center mb-14">
            ...
            <button
              className="focus:outline-none  bg-yellow-400 hover:bg-yellow-500 
                                focus:ring-4 focus:ring-yellow-300 font-bold rounded-lg text-lg px-[40px] py-5 me-2 mb-2
            ...
            <tbody>
              {purchaseorderList.map((item, index) => (
                ...
                >
                  ...
                  <td className="text-center py-4">
                    <button
                      className="bg-yellow-400 hover:bg-yellow-500  font-bold rounded-lg py-2 px-4 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                      onClick={() => pohandleChange(item.id)}
                    >
                      승인
                    </button>
                  </td>
