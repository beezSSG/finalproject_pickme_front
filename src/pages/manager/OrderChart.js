...
function OrderChart() {
  ...
  function orderchart() {
    ...
    if (address) {
      // 광역시 또는 구 중 하나만 선택된 경우에 요청을 보냄
      axios
        .get("manager/orderchart", {
          params: { date: selectedDate, address: address },
        })
        .then(function (resp) {
  ...
  // 페이지로 이동하는 함수들
  function goPo() {
    navigate("/manager/managerpurchaseorder");
  }
  function goContact() {
    navigate("/contactus");
  }
        ...
        <div className="flex-1 p-10">
          <div className="py-[25px] px-[25px] bg-[#ebedf4] rounded-xl">
            <div className="flex items-center justify-between">
              <h1 className="text-[#5a5c69] text-[28px] leanding-[34px] font-normal cursor-pointer">
                관리자 홈
              </h1>
              <button
            ...
            {open && (
              <div className="grid grid-cols-4 gatp-[30px] mt-[25px] pb-[15px]">
                {categorycountList.map((item, index) => (
                  <div
                    key={index}
                    className="h-[100px] rounded-[8px] bg-white border-l-[4px] border-yellow-500 flex items-center justify-between px-[30px] cursor-pointer hover:shoadow-lg transform hover:scale-[103%] transition duration-300 ease-in-out"
                  >
                    <div className="w-full">
                      <h2 className="text-gray-700 text-[20px] leading-[17px] font-bold">
                        {item.category}
                      </h2>
                  ...
                  <div className="bg-[#f8f9fc] flex items-center justify-between py-[15px] px-[20px] border-b-[1px] border-[#ededed] mb-[20px]">
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute top-[35px] right-0 bg-white border border-gray-200 shadow-md rounded-md p-2">
                        ...
                        {/* 광역시 선택 */}
                        <select
                          className="w-full p-1 mt-2 border border-gray-300 rounded-md"
            ...
            {!open && (
              <div className="grid grid-cols-2 gap-[30px] mt-[25px] pb-[15px] gap-y-28">
                <div
                  className={`h-[150px] rounded-[8px] bg-white flex items-center justify-between px-[30px] cursor-pointer hover:shadow-lg transform hover:scale-[103%] transition duration-300 ease-in-out ${
                    notanswerCcb === 0
                      ? "border-l-[6px] border-green-700"
                      : "border-l-[6px] border-red-700"
                  }`}
                  onClick={goContact}
                >
                  <div className="w-full">
                    <h2
                      className={`text-gray-700 text-[30px] leading-[17px] font-bold ${
                        notanswerCcb === 0 ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      {notanswerCcb === 0 ? (
                ...
                >
                  <div className="w-full">
                    <h2
                      className={`text-gray-700 text-[30px] leading-[17px] font-bold ${
                        notPo === 0 ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      {notPo === 0 ? (
                        <GoCheck className="text-green-600" />
                ...
                >
                  <div className="w-full">
                    <h2
                      className={`text-gray-700 text-[30px] leading-[17px] font-bold ${
                        threeCount === 0 ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      {threeCount === 0 ? (
                        <GoCheck className="text-green-600" />
                ...
                >
                  <div className="w-full">
                    <h2
                      className={`text-gray-700 text-[30px] leading-[17px] font-bold ${
                        notocr === 0 ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      {notocr === 0 ? (
                        <GoCheck className="text-green-600" />
